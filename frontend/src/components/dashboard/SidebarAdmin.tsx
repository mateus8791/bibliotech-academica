 ...
 * =====================================================
 * Menu lateral customizado para perfil de ADMIN
 * Acesso completo + Gestão de Permissões
...
import { sidebarConfigAdmin, filterLinksByPermissions, SidebarSubItem } from '@/config/sidebarConfig';
...
export const SidebarAdmin = () => {
  ...
  // Obtém permissões do usuário logado (assumindo que no futuro você vai mapear isso melhor no contexto, aqui pegamos um mock ou liberamos tudo se for admin)
  const userPermissions = useMemo(() => {
    // Um Admin logado teoricamente tem todas as permissões. 
    // Em um sistema real, você extrai isso de usuario.permissoes
    return ['can_view_reports', 'can_view_books', 'can_view_loans', 'can_view_users', 'can_view_logs', 'can_manage_roles', 'can_manage_domains'];
  }, []);

  const navLinks = useMemo(() => {
    return filterLinksByPermissions(sidebarConfigAdmin, userPermissions);
  }, [userPermissions]);
        ...
        {isCollapsed ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-blue-600 dark:text-blue-400 font-semibold uppercase tracking-wide">Admin</span>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="relative group flex-shrink-0">
            {usuario?.foto_url ? (
              ...
              <div className={`${isCollapsed ? 'w-10 h-10 text-sm' : 'w-12 h-12 text-base'} rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-bold shadow-md ring-2 ring-blue-200 dark:ring-blue-700`}>
                {getInitials(usuario?.nome || 'Admin')}
              </div>
            )}
            {/* Indicador online */}
          ...
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-blue-600 dark:text-blue-400 uppercase tracking-wide font-medium">
                {usuario?.tipo_usuario === 'admin' ? 'Administrador' : usuario?.tipo_usuario || 'Admin'}
              </p>
            </div>
          )}

          {/* Sino de notificações real (apenas quando expandido) */}
          {!isCollapsed && (
