...
/**
 * Sidebar unificada para Admin e Bibliotecário.
 * - Admin: exibe todos os links, incluindo Logs e Domínios
 * - Bibliotecário: exibe apenas os links compartilhados
 */
export const Sidebar = () => {
  ...
  const roleLabel = isAdmin ? 'Administrador' : 'Bibliotecário';
  ...
  const navLinks = useMemo(() => {
    ...
    return isAdmin ? [...shared, ...adminOnly, ...profile] : [...shared, ...profile];
  }, [isAdmin]);
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className={`${isCollapsed ? 'p-4' : 'p-6'} flex items-center justify-center`}>
        {isCollapsed ? (
          <Image src="/logo.png" alt="BiblioTech" width={40} height={40} sizes="40px" className="rounded-lg" />
        ) : (
          <div className="flex items-center gap-3">
            <Image src="/logo.png" alt="BiblioTech" width={48} height={48} sizes="48px" className="rounded-lg" />
            <div className="flex flex-col">
              <span className="text-xl font-bold text-gray-900 dark:text-white leading-tight">BiblioTech</span>
              <span className="text-xs text-blue-600 dark:text-blue-400 font-semibold uppercase tracking-wide">
                {isAdmin ? 'Admin' : 'Bibliotecário'}
              </span>
            </div>
          </div>
        )}
      ...
      {/* User */}
      <div className={`${isCollapsed ? 'px-2 pb-4' : 'px-4 pb-6'} border-b border-gray-200 dark:border-gray-800`}>
        <div className={`flex ${isCollapsed ? 'flex-col items-center gap-2' : 'items-center gap-3'}`}>
          <div className="relative flex-shrink-0">
            ...
            <div className={`absolute ${isCollapsed ? 'bottom-0 right-0' : 'bottom-0 right-0.5'} w-3 h-3 bg-green-500 rounded-full border-2 border-white dark:border-gray-900`}></div>
          </div>

          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-gray-900 dark:text-white truncate text-sm">
                {usuario?.nome || roleLabel}
              </h3>
      ...
      <div className={`${isCollapsed ? 'p-2' : 'p-4'} border-t border-gray-200 dark:border-gray-800 space-y-2`}>
        <button
          onClick={toggleTheme}
          className={`flex items-center gap-3 px-4 py-3 rounded-lg w-full text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 transition-all ${isCollapsed ? 'justify-center' : ''}`}
          title={theme === 'light' ? 'Ativar modo escuro' : 'Ativar modo claro'}
        >
          {theme === 'light' ? <Moon className="w-5 h-5 flex-shrink-0" /> : <Sun className="w-5 h-5 flex-shrink-0" />}
          {!isCollapsed && <span className="font-medium">{theme === 'light' ? 'Modo Escuro' : 'Modo Claro'}</span>}
        </button>

        <button
          onClick={logout}
          className={`flex items-center gap-3 px-4 py-3 rounded-lg w-full text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20 transition-all ${isCollapsed ? 'justify-center' : ''}`}
        >
      ...
      <button
        ...
        className="absolute -right-3 top-6 w-6 h-6 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-full flex items-center justify-center text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all z-50 shadow-md"
      >
